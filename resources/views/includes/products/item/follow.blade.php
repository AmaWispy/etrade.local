...
<div class="w-full" x-data="{open:false, follow:false}">
    <div x-bind:class=" follow ? 'hidden' : 'inline-flex'" class="justify-between items-center w-full">
        <!-- Desk design Start -->
            <div class="justify-between items-center w-full xl:inline-flex hidden">
                <div class="inline-flex items-center 2xl:gap-6 xl:gap-3 w-full">
                    ...
                    <div>
                        <a href="{{ route('shop.card', ['slug' => $product->slug, 'id' => $product->id]) }}" class="h-full w-full">
                    ...
                    <div class="text-xl 2xl:w-80 text-ellipsis xl:w-72 font-medium  truncate">
                        <a href="{{ route('shop.card', ['slug' => $product->slug, 'id' => $product->id]) }}">{{ $product->name }}</a>
                    </div>
                </div>
        ...
        <!-- Mobile design Start-->
            <div class="xl:hidden inline-flex gap-4 w-full">
                <div class="h-20 w-24">
                    <a href="{{ route('shop.card', ['slug' => $product->slug, 'id' => $product->id]) }}" class="w-full h-full">
                ...
                <div class="w-full">
                    <div class="w-full">
                        <div class="flex justify-between items-center w-full border-b py-2">
                            <a href="{{ route('shop.card', ['slug' => $product->slug, 'id' => $product->id]) }}" class="lg:text-xl md:text-lg truncate sm:w-36 md:w-60 lg:w-auto ">{{ $product->name }}</a>
                            <div 
                            class="bg-neutral-100 rounded-full w-fit h-fit px-2 py-1 "
                            x-on:click="follow = !follow">
                                <a href="#" 
